...
def plot_characteristics(
    ...
    use_km: bool = True,
    use_minutes: bool = True,
    title: str = "Characteristic Curves (x-t diagram)",
    show: bool = True,
    savepath: Optional[str] = None
) -> plt.Figure:
    """Plot the theoretical characteristic lines in the s-t plane."""
    ...
    for P, v in zip(P_vals, wave_speeds):
        ...
        ax.plot(x_plot, y_plot, color='black', alpha=0.4, linewidth=0.8)
...
def plot_shock_trajectory(
    ...
    """Compare the numerical shock position with the theoretical Rankine-Hugoniot speed."""
    t, s, A_hist = out["t"], out["s"], out["A"]

    s_num = np.zeros_like(t)
    s_RH = np.zeros_like(t)

    # Starting positions based on the initial condition
    s_num[0] = ic.s0
    s_RH[0] = ic.s0

    # The analytical flux function for the rectangular canyon
    def Q_rect(A_val):
    ...
    for k in range(1, len(t)):
        ...
        # 1. Find Numerical Shock (Location of the steepest negative gradient)
        grad = np.diff(A_hist[k])
        idx_shock = int(np.argmin(grad))
        s_num[k] = s[idx_shock]

        # 2. Calculate Theoretical R-H Speed based on current peak and base
        A_L = np.max(A_hist[k])  # The peak area behind the shock (c1)
        A_R = ic.A_base  # The background area ahead of the shock (c0)
        ...
        # Integrate the theoretical speed to get position
        s_RH[k] = s_RH[k - 1] + v_RH * dt
    ...
    ax.plot(x_num, y_plot, color='blue', linewidth=3, label="Godunov Numerical Shock")
    ax.plot(x_RH, y_plot, color='red', linestyle='--', linewidth=2, label="Rankine-Hugoniot Theory")
